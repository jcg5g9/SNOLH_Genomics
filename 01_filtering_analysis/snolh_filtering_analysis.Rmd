...
load("../01_filtering_analysis/data/processed_raw_data/mdata_01_filter_shoal.Rda")
...
```{r}
# Load genotype data (gdata) with SNP filter 
load("data/processed_raw_data/gdata_03_filter_samples.Rda")

# Load metadata (mdata) with sample filter 
load("data/processed_raw_data/mdata_03_filter_samples.Rda")
...
# Get counts of "no-genotype" calls per site across samples
filtered_snp_missing <- apply(filtered_data[13:198], 2, function(x) {length(which(x=="unknown"))})
...
# Bind with sample ID
filtered_sample_missing <- cbind(filtered_data[,c(1,4)], filtered_sample_missing)
...
# Bind with sample ID
spb_smbc_sample_missing <- cbind(filtered_data[,c(1,4)], spb_smbc_sample_missing)
...
##### Summarize genotype missingness by genetic group and plot: `figures/cih_missing.pdf`
```{r}
# Load sample missing data 
