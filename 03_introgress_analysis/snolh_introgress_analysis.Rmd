...
### STEP 1: Conduct introgression analysis for hybridization between SPB and SMBC.
...
#### 1a: Gather, clean, and merge hybrid metadata with full data for SPB and SMBC; run the Rmd chunk below.
...
load("../02_structure_analysis/data/newhybrids_data/output_data/spb_smbc_output/spb_smbc_pofz.rda")
...
# save modified full data for downstream analyses
save(full_data, file = "data/full_data.Rda")

# save metadata for downstream analyses
save(spb_smbc_hybrid_metadata, file = "data/spb_smbc_hybrid_data.Rda")

# save hybrid for downstream analyses
save(spb_smbc_hybrid_data, file = "data/spb_smbc_hybrid_data.Rda")
```

#### 1b: Prepare hybrid data for introgression analysis; run the Rmd chunk below.

##### Prepare hybrid data:
```{r}
# load metadata
load("data/spb_smbc_hybrid_metadata.Rda")

# load hybrid data
load("data/spb_smbc_hybrid_data.Rda")
...
# convert hybrid categories to factors
spb_smbc_hybrid_category <- factor(spb_smbc_hybrid_data[,3])

# Select only SPB-SNPs from hybrid data
...
# add add column to hold "c" variable
spb_smbc_loci_data <- cbind(spb_smbc_loci_data, rep("c", times = 15))

# change column names
colnames(spb_smbc_loci_data) <- c("locus","type")

# save spb_smbc loci data for downstream analysis
save(spb_smbc_loci_data, file = "data/spb_smbc_loci_data.Rda")
...
# save hybrid genotypes for downstream analyses
save(spb_smbc_hybrid_genotypes, file = "data/spb_smbc_hybrid_genotypes.Rda")
...
# Transpose p1 object created above for introgress input format
spb_p1_data <- t(spb_p1_data)

# Transpose p2 object created above for introgress input format
smbc_p2_data <- t(smbc_p2_data)

# Transpose admix object created above for introgress input format
spb_smbc_admix_data <- t(spb_smbc_admix_data)

# save p1 object for downstream analyses
save(spb_p1_data, file = "data/spb_p1_data.Rda")

# save p2 object for downstream analyses
save(smbc_p2_data, file = "data/smbc_p2_data.Rda")

# save admix object for downstream analyses
save(spb_smbc_admix_data, file = "data/spb_smbc_admix_data.Rda")
```

#### 1c: Run introgress analysis; run the Rmd chunk below.
...
# load hybrid genotypes
load("data/spb_smbc_hybrid_genotypes.Rda")

# load p1 object
load("data/spb_p1_data.Rda")

# load p2 object
load("data/smbc_p2_data.Rda")

# load admix object
load("data/spb_smbc_admix_data.Rda")

# load loci data 
load("data/spb_smbc_loci_data.Rda")
...
# save spb_smbc introgress data 
save(spb_smbc_introgress_data, file = "data/spb_smbc_introgress_data.Rda")
```

#### 1d: Plot introgression analysis results using a triangle plot (relationship between hybrid index and interspecific heterozygosity); run the Rmd chunk below.

##### Generate triangle plot for SPB and the SMBC: `02_spb_smbc_introgress.pdf`
```{r}
# load spb_smbc introgress data 
load("data/spb_smbc_introgress_data.Rda")
...
### STEP 2: Conduct introgression analysis for hybridization between SMB and CIH.
...
#### 2a. Gather, clean, and merge hybrid metadata with full data for SMB and the CIH; run the Rmd chunk below.
...
# save metadata for downstream analyses
save(smb_cih_hybrid_metadata, file = "data/smb_cih_hybrid_metadata.Rda")

# save hybrid data for downstream analyses
save(smb_cih_hybrid_data, file = "data/smb_cih_hybrid_data.Rda")
```

#### 2b: Prepare hybrid data for introgression analysis; run the Rmd chunk below.

##### Prepare hybrid data:
```{r}
# load metadata for downstream analyses
load("data/smb_cih_hybrid_metadata.Rda")

# load hybrid data for downstream analyses
load("data/smb_cih_hybrid_data.Rda")
...
# save hybrid genotypes for downstream analyses
save(smb_cih_hybrid_genotypes, file = "data/smb_cih_hybrid_genotypes.Rda")

# Create data object for parent 1 genotypes (SPB)
smb_p1_data <- smb_cih_hybrid_genotypes %>%
  filter(pop == "pure_smb") %>%
  dplyr::select(-c(pop))

# Create data object for parent 2 genotypes (IH)
cih_p2_data <- smb_cih_hybrid_genotypes %>%
  filter(pop == "pure_cih") %>%
  dplyr::select(-c(pop))
...
# Transpose p1 object created above for introgress input format
smb_p1_data <- t(smb_p1_data)

# Transpose p2 object created above for introgress input format
cih_p2_data <- t(cih_p2_data)

# Transpose admix object created above for introgress input format
smb_cih_admix_data <- t(smb_cih_admix_data)

# save p1 object for downstream analyses
save(smb_p1_data, file = "data/smb_p1_data.Rda")

# save p2 object for downstream analyses
save(cih_p2_data, file = "data/cih_p2_data.Rda")

# save admix object for downstream analyses
save(smb_cih_admix_data, file = "data/smb_cih_admix_data.Rda")
```

#### 2c: Run introgress analysis; run the Rmd chunk below.
...
# load hybrid genotypes
load("data/smb_cih_hybrid_genotypes.Rda")

# load p1 object
load("data/smb_p1_data.Rda")

# load p2 object
load("data/cih_p2_data.Rda")

# load admix object
load("data/smb_cih_admix_data.Rda")

# load loci data 
load("data/smb_cih_loci_data.Rda")

# Generate introgress input formatted data
smb_cih_introgress <- prepare.data(admix.gen = smb_cih_admix_data, 
                                   loci.data = smb_cih_loci_data, 
                                   parental1 = smb_p1_data,
                                   parental2 = smb_p2_data, 
                                   pop.id = F, 
                                   ind.id = F, 
                                   fixed = FALSE, 
                                   sep.rows = FALSE, 
...
# save smb_cih introgress data for downstream analysis
save(smb_cih_introgress_data, file = "data/smb_cih_introgress_data.rda")
```

#### 2d: Plot introgression analysis results using a triangle plot (relationship between hybrid index and interspecific heterozygosity); run the Rmd chunk below.

##### Generate triangle plot for SMB and the CIH: `03_smb_cih_introgress.pdf`
```{r}
# load spb_cih introgress data 
load("data/spb_cih_introgress_data.Rda")
...
### STEP 3: Conduct introgression analysis for hybridization between LRB and OB
...
#### 3a. Gather, clean, and merge hybrid metadata with full data for LRB and OB; run the Rmd chunk below.

##### Prepare hybrid metadata:
```{r}
# Load in saved .Rda file with probabilities for lrb and ob
...
# save metadata for downstream analyses
save(lrb_ob_hybrid_metadata, file = "data/lrb_ob_hybrid_metadata.Rda")

# save hybrid data for downstream analyses
save(lrb_ob_hybrid_data, file = "data/lrb_ob_hybrid_data.Rda")
```

#### 3b: Prepare hybrid data for introgression analysis; run the Rmd chunk below.
...
# save hybrid genotypes for downstream analyses
save(lrb_ob_hybrid_genotypes, file = "data/lrb_ob_hybrid_genotypes.Rda")
...
# Transpose p1 object created above for introgress input format
lrb_p1_data <- t(lrb_p1_data)

# Transpose p2 object created above for introgress input format
ob_p2_data <- t(ob_p2_data)

# Transpose admix object created above for introgress input format
lrb_ob_admix_data <- t(lrb_ob_admix_data)

# save p1 object for downstream analyses
save(lrb_p1_data, file = "data/lrb_p1_data.Rda")

# save p2 object for downstream analyses
save(ob_p2_data, file = "data/ob_p2_data.Rda")

# save admix object for downstream analyses
save(lrb_ob_admix_data, file = "data/lrb_ob_admix_data.Rda")
```

#### 3c: Run introgress analysis; run the Rmd chunk below.

##### Run introgression analysis:
```{r}
# load metadata
load("data/lrb_ob_hybrid_data.Rda")

# load hybrid data
load("data/lrb_ob_hybrid_data.Rda")

# load hybrid genotypes
load("data/lrb_ob_hybrid_genotypes.Rda")

# load p1 object
load("data/lrb_p1_data.Rda")

# load p2 object
load("data/ob_p2_data.Rda")

# load admix object
load("data/lrb_ob_admix_data.Rda")

# load loci data 
load("data/lrb_ob_loci_data.Rda")
