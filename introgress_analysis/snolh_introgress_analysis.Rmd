...
spb_smbc_hybrid_category <- factor(spb_smbc_hybrid_data[,2])
...
ggplot(spb_smbc_introgress_data, aes(x = h, y = int_het, fill = hybrid_category)) + 
...
smb_cih_hybrid_category <- factor(smb_cih_hybrid_data[,2])
...
ggplot(smb_cih_introgress_data, aes(x = h, y = int_het, fill = hybrid_category)) + 
  geom_errorbar(aes(xmin=lower, xmax=upper), width = .1) +
  geom_point(alpha = 0.8, 
             color = "black", 
             pch = 21, 
...
spb_smbc_sim <- spb_smbc_hybrid_data %>%
  dplyr::select(hybrid_category, Structure_Num, matches("SM_SP"))
...
spb_smbc_sim_metadata <- c(rep("Simulated Pure SMBC", times = 200),
                           rep("Simulated Pure SPB", times = 200),
                           rep("Simulated F1", times = 200),
                           rep("Simulated F2", times = 200),
                           rep("Simulated BC SMBC", times = 200),
                           rep("Simulated BC SPB", times = 200))

# Convert to dataframe and name column
spb_smbc_sim_metadata <- as.data.frame(spb_smbc_sim_metadata)
colnames(spb_smbc_sim_metadata) <- "hybrid_category"
...
levels(spb_smbc_emp_data$hybrid_category) <- c("Real BC SMBC", "Real BC SPB", "Real F1", "Real F2", "Real SMBC", "Real SPB")
...
#### 1h: Run DAPC and plot results; run the Rmd chunk below:
...
spb_smbc_dapc_lds <- as.data.frame(spb_smbc_dapc$ind.coord)
spb_smbc_dapc_lds <- cbind(data.frame(spb_smb_all_metadata), spb_smbc_dapc_lds)
colnames(spb_smbc_dapc_lds) <- c("pop","LD1","LD2")

pdf("figures/02_spb_smbc_dapc.pdf", width=8, height=6)

ggplot(spb_smbc_dapc_lds, aes(x = LD1, y = LD2, fill = pop)) +
   geom_point(aes(fill = pop), color = "black", pch = 21, size = 5, show.legend = F) +
   labs(x = "LD1", y = "LD2", fill = "Hybrid Category") +
   scale_fill_manual(values = c("grey90","grey18","grey35","grey60","grey100","grey5","deepskyblue","deeppink2","sienna4","lightgreen","orchid1")) +
   theme_set(theme_cowplot(12)) +
...
scatter(spb_smbc_dapc, 
        col = c("grey90","grey18","grey35","grey60","grey100","grey5","deepskyblue","deeppink2","sienna4","lightgreen","orchid1"),
        scree.da = T,
        scree.pca = T,
        posi.da = "bottomright",
        posi.pca = "bottomleft",
...
dapc_midark_white_admix_plot <- ggplot(dapc_midark_white_admix_LDs, aes(x = LD1, y = LD2, fill = pop)) +
   geom_point(aes(fill = pop), color = "black", pch = 21, size = 5, show.legend = F) +
   labs(x = "LD1", y = "LD2", fill = "Hybrid Category") +
   scale_fill_manual(values = c("grey5","deepskyblue","grey18","grey60","grey35","grey90","grey100","deeppink2","sienna4","lightgreen","orchid1")) +
   theme_set(theme_cowplot(12)) +
   theme(legend.title = element_text(size = 20)) +
   theme(legend.title = element_text(face = "bold")) +
   theme(legend.text = element_text(size = 20)) +
