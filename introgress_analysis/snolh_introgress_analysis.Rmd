...
In this phase of the analysis, we read in raw SNP genotype data along with hybrid category inference from NewHybrids analysis (see `snolh_structure_analysis.Rmd`, Phase 2 - 4, Step 3) for each hierarchical analysis conducted in Structure: 1) Spotted Bass (SPB) vs. all species in the Smallmouth Bass species complex (SBSC); 2) Smallmouth Bass (SMB) vs. all other species in the Central Interior Highlands; 3) all species within the Central Interior Highlands
...
#### 1b. Read in and prepare data for hybridization between SPB and all other species in the Smallmouth Bass species complex (`snolh_structure_analysis.Rmd`, Phase 1, Step 2).

##### 1b.1. Load in data; run the Rmd chunk below.

##### Load in saved .Rda file
```{r}
load("../structure_analysis/data/newhybrids_data/output_data/spb_other_output/spb_other_pofz.rda")
```
...
##### Prepare data:
```{r}
# Get pure SMBC individuals (pure Smallmouth Bass species complex)
spb_other_pure_ih_metadata <- spb_other_pofz %>%
  filter(hybrid_category == "pure_ih") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Get pure SPB individuals
spb_other_pure_spb_metadata <- spb_other_pofz %>%
  filter(hybrid_category == "pure_spb") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Get F1 individuals
spb_other_f1_metadata <- spb_other_pofz %>%
  filter(hybrid_category == "f1") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Get F2 individuals
spb_other_f2_metadata <- spb_other_pofz %>%
  filter(hybrid_category == "f2") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Get IH backcross individuals
spb_other_bc_ih_metadata <- spb_other_pofz %>%
  filter(hybrid_category == "bc_ih") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Get SPB backcross individuals
spb_other_bc_spb_metadata <- spb_other_pofz %>%
  filter(hybrid_category == "bc_spb") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Generate list of all individuals by hybrid category
hybrid_metadata <- rbind(spb_other_pure_ih_metadata, 
                           spb_other_pure_spb_metadata, 
                           spb_other_f1_metadata, 
                           spb_other_f2_metadata, 
                           spb_other_bc_ih_metadata, 
                           spb_other_bc_spb_metadata)
...
##### Generate triangle plot for SPB and all other species in the Smallmouth Bass species complex: `02_spb_other_introgress.pdf`
```{r}
# Generate triangle plot, regressing interspecific heterozygosity on hybrid index
pdf("figures/02_spb_other_introgress.pdf", width=8, height=6)
...
#### 1c. Read in and prepare data for hybridization between SMB and species in the Central Interior Highlands (`snolh_structure_analysis.Rmd`, Phase 1, Step 3).

##### 1c.1. Load in data; run the Rmd chunk below.

##### Load in saved .Rda file
```{r}
load("../structure_analysis/data/newhybrids_data/output_data/smb_ih_output/smb_ih_pofz.rda")
```
...
```{r}
# Get pure SMBC individuals (pure Smallmouth Bass species complex)
smb_ih_pure_ih_metadata <- smb_ih_pofz %>%
  filter(hybrid_category == "pure_ih") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Get pure SPB individuals
smb_ih_pure_smb_metadata <- smb_ih_pofz %>%
  filter(hybrid_category == "pure_smb") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Get F1 individuals
smb_ih_f1_metadata <- smb_ih_pofz %>%
  filter(hybrid_category == "f1") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Get F2 individuals
smb_ih_f2_metadata <- smb_ih_pofz %>%
  filter(hybrid_category == "f2") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Get IH backcross individuals
smb_ih_bc_ih_metadata <- smb_ih_pofz %>%
  filter(hybrid_category == "bc_ih") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Get SPB backcross individuals
smb_ih_bc_smb_metadata <- smb_ih_pofz %>%
  filter(hybrid_category == "bc_smb") %>%
  filter(probability > 0.5) %>%
  dplyr::select(Structure_Num, hybrid_category)

# Generate list of all individuals by hybrid category
hybrid_metadata <- rbind(smb_ih_pure_ih_metadata, 
                           smb_ih_pure_smb_metadata, 
                           smb_ih_f1_metadata, 
                           smb_ih_f2_metadata, 
                           smb_ih_bc_ih_metadata, 
                           smb_ih_bc_smb_metadata)
...
##### Generate triangle plot for SMB and species in the CIH: `03_smb_ih_introgress.pdf`
```{r}
# Generate triangle plot, regressing interspecific heterozygosity on hybrid index
pdf("figures/03_smb_other_introgress.pdf", width=8, height=6)
...
#### 1d. Read in and prepare data for hybridization among species in the Central Interior Highlands (`snolh_structure_analysis.Rmd`, Phase 1, Step 4).
We found no hybrid individuals when we tested for hybridization between Neosho Bass and Little River Bass or between Neosho Bass and Ouachita Bass. However, we found one hybrid when we assessed hybridization between Little River Bass and Ouachita Bass; introgression analysis for this individual is below.

##### 1d.3. Read in and prepare data for hybridization between Little River Bass and Ouachita Bass; run the Rmd chunk below.

##### Load in saved .Rda file
```{r}
load("../structure_analysis/data/newhybrids_data/output_data/little_ouachita_output/little_ouachita_pofz.rda")
```
