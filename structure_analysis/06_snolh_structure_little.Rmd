---
title: "Population Structure of Little River Bass"
author: "Joe Gunn"
date: "2022-08-14"
...
## Specific Aim: Hierarchical structure in Little River Bass
<font size="+1">For this aim, we are using Bayesian inference with STRUCTURE and NEWHYBRIDS to assess population structure in the Little River Basin (Little River Bass)
...
In this step, we are filtering the metadata and SNP genotype data to only include samples of interest for analysis of population structure of Little River Bass. For this analysis, these data will include Little River Bass and only SNP loci that were found to be diagnostic for Little River Bass.

##### 1b.1. Filter metadata; run the Rmd chunk below:

##### Filter metadata to only include Ouachita Bass
```{r}
little_metadata <- ouachita_little_metadata %>%
  ...
  filter(Putative_Taxon != "Ouachita_OuachitaRiver") # Only Little River Bass
```
...
```{r}
# Little River Bass only:
little <- cbind(little_metadata, little_snps)

# Save combined datasets .Rda files to be loaded in downstream analysis
save(little, file = "data/processed_genotype_data/little.rda")
...
write_xlsx(little, path = "data/processed_genotype_data/little.xlsx")
```
...
```{r}
# Generate structure file for Little River Bass only
genepop_structure("data/genepop_data/little_genepop.txt", 
                  locusnames = TRUE, 
                  #popgroup = "data/genepop_data/little_pops.csv",
                  path = "data/structure_data/input_data/little_structure.txt")
...
##### Generate batch commands: `code/structure_code/batch_cmd_lists/little_structure_batch_cmd_list.txt`
```{r}
# N(pops) = 10
...
cat("", file="code/structure_code/batch_cmd_lists/little_structure_batch_cmd_list.txt")
...
for(ii in 1:nrow(nk)) {
  ...
  for(aa in 1:nrow(nreps)) {
    ...
    param_files <- paste(" -m /home/jcg5g9/data/SNOLH_Genomics/structure_analysis/data/structure_data/param_files/mainparams_little -e /home/jcg5g9/data/SNOLH_Genomics/structure_analysis/data/structure_data/param_files/extraparams")

    input <- paste(" -i /home/jcg5g9/data/SNOLH_Genomics/structure_analysis/data/structure_data/input_data/little_structure.txt")
  
    output <- paste(" -o /home/jcg5g9/data/SNOLH_Genomics/structure_analysis/data/structure_data/output_data/little_structure_output/")
  
    cat(paste(structure_call, "-K ", nk[ii,], param_files, input, output,
            strsplit("little_structure.txt", "_structure.txt", fixed=TRUE)[[1]][1], "_", nk[ii,], "_", nreps[aa,], sep=""),
      "\n", 
      file=paste("code/structure_code/batch_cmd_lists/little_structure_batch_cmd_list.txt"), # name file to store list
      append=TRUE)
  }
...
##### Generate batch commands: `code/structure_code/shell_scripts/little_structure.sh`
```{r}
#!/bin/bash
#-------------------------------------------------------------------------------
#  SBATCH CONFIG
...
#COMMANDA=`head -n ${SLURM_ARRAY_TASK_ID} ../batch_cmd_lists/ittle_structure_batch_cmd_list.txt | tail -n 1`
#eval $COMMANDA
...
`data/structure_data/param_files/mainparams_little` <br> 
...
<b>Little River Bass populations only:</b> 10 <br>
...
<b>Little River Bass populations only:</b> 241 <br>

###### 1f.1.4. Set the number of loci. These values should be the following:

<b>Little River Bass populations only:</b> 46 <br>
