---
title: "Hybridization between Neosho Bass, Ouachita Bass, and Little River Bass"
author: "Joe Gunn"
date: "2022-08-13"
...
<font size="+1">For this aim, we are using Bayesian inference with STRUCTURE and NEWHYBRIDS to assess patterns and levels of hybridization between Neosho Bass (<i>M. velox</i>), and two potentially distinct species in the Ouachita River Basin (Ouachita Bass and Little River Bass).
=======
...
load("data/newhybrids_data/hybrid_data/smb_ih_hybrids.rda")
```
...
```{r}
ih_metadata <- metadata %>%
  filter(Putative_Taxon != "Spotted") %>%
  filter(Putative_Taxon != "Smallmouth_Northern") %>%
  filter(Putative_Taxon != "Smallmouth_TN_Native") %>%
  filter(Putative_Taxon != "Smallmouth_TNLakeStrain") # Only Interior Highland Bass

ih_metadata <- ih_metadata[ ! ih_metadata$Structure_Num %in% smb_ih_hybrids, ] # Remove hybrids detected with Spotted Bass
```
...
```{r}
## All interior Highlands Species (Neosho, Ouachita River, Little River)
ih_snps <- full_data %>%
  filter(Putative_Taxon != "Spotted") %>%
  filter(Putative_Taxon != "Smallmouth_Northern") %>%
  filter(Putative_Taxon != "Smallmouth_TN_Native") %>%
...
ih_snps <- ih_snps[ ! ih_snps$Structure_Num %in% smb_ih_hybrids, ] # Remove hybrids detected with Spotted Bass
...
save(ih, file = "data/processed_genotype_data/ih.rda")
...
##### Generate batch commands: `code/structure_code/batch_cmd_lists/ih_structure_batch_cmd_list.txt`
```{r}
# N(pops) = 18
...
cat("", file="code/structure_code/batch_cmd_lists/ih_structure_batch_cmd_list.txt")
...
for(ii in 1:nrow(nk)) {
  ...
  for(aa in 1:nrow(nreps)) {
    ...
    cat(paste(structure_call, "-K ", nk[ii,], param_files, input, output,
      ...
      "\n", 
      file=paste("code/structure_code/batch_cmd_lists/ih_structure_batch_cmd_list.txt"), # name file to store list
      append=TRUE)
  }
...
##### Generate batch commands: `code/structure_code/shell_scripts/ih_structure.sh`
```{r}
#!/bin/bash
#-------------------------------------------------------------------------------
#  SBATCH CONFIG
...
<b>Interior Highlands populations only:</b> 18 <br>
...
<b>Interior Highlands populations only:</b> 450 <br>

###### 1f.1.4. Set the number of loci. These values should be the following:

<b>Interior Highlands populations only:</b> 138 <br>
...
To parse the dataset, we manually altered the IH genepop Excel file (`data/genepop_data/ih_genepop.xlsx`) to include only the individuals for each pairwise analysis, and we saved the associated Excel and txt files:
  ...
  1) `data/genepop_data/ih_neosho_little_genepop.xlsx` and  `data/genepop_data/ih_neosho_little_genepop.txt`
  2) `data/genepop_data/ih_neosho_ouachita_genepop.xlsx` and  `data/genepop_data/ih_neosho_ouachita_genepop.txt`
  3) `data/genepop_data/ih_little_ouachita_genepop.xlsx` and  `data/genepop_data/ih_little_ouachita_genepop.txt`
...
Running the Rmd chunk below generates 6 files: 
  1) NewHybrids input for Neosho Bass and Little River Bass (`ih_neosho_little_nh.txt`),
  2) accompanying individual file for Neosho Bass and Little River Bass (`ih_neosho_little_individuals.txt`)
  3) NewHybrids input for Neosho Bass and Ouachita Bass (`ih_neosho_ouachita_nh.txt`),
  4) accompanying individual file for Neosho Bass and Ouachita Bass (`ih_neosho_ouachita_individuals.txt`)
  5) NewHybrids input for Little River Bass and Ouachita Bass (`ih_little_ouachita_nh.txt`),
  6) accompanying individual file for Little River VBassand Ouachita Bass (`ih_little_ouachita_individuals.txt`)
...
genepop_newhybrids("data/genepop_data/ih_neosho_little_genepop.txt", 
                   path = "data/newhybrids_data/input_data/ih_neosho_little_nh.txt")

# Generate newhybrids files for Neosho Bass and Ouachita Bass
genepop_newhybrids("data/genepop_data/ih_neosho_ouachita_genepop.txt", 
                   path = "data/newhybrids_data/input_data/ih_neosho_ouachita_nh.txt")

# Generate newhybrids files for Little River Bass and Ouachita Bass
genepop_newhybrids("data/genepop_data/ih_little_ouachita_genepop.txt", 
                   path = "data/newhybrids_data/input_data/ih_little_ouachita_nh.txt")
...
ih_neosho_little_individuals <- data.frame(indiv = as.character(ih_neosho_little_metadata$Structure_Num))

# Generate individual file (list of sample names ('Structure_Num') to accompany NewHybrids for Neosho Bass and Ouachita Bass)
ih_neosho_ouachita_individuals <- data.frame(indiv = as.character(ih_neosho_ouachita_metadata$Structure_Num))
...
ih_little_ouachita_individuals <- data.frame(indiv = as.character(ih_little_ouachita_metadata$Structure_Num))

# Write individual files to .txt format
write_tsv(ih_neosho_little_individuals, "data/newhybrids_data/input_data/ih_neosho_little_individuals.txt")
write_tsv(ih_neosho_ouachita_individuals, "data/newhybrids_data/input_data/ih_neosho_ouachita_individuals.txt")
write_tsv(ih_little_ouachita_individuals, "data/newhybrids_data/input_data/ih_little_ouachita_individuals.txt")
```
...
##### 3c.1. Manually rename output directory: `ih_neosho_little_output/`, ``ih_neosho_ouachita_output/`, `ih_little_ouachita_output/`
...
##### Visualize NEWHYBRIDS output and generate plots for all comparisons: `ih_neosho_little.pdf`, `ih_neosho_ouachita.pdf`, `ih_little_ouachita.pdf`
```{r}
# Read in NewHybrids "PofZ" output txt file for Neosho and Little River Bass
ih_neosho_little_pofz <- read_tsv("data/newhybrids_data/output_data/ih_neosho_little_output/ih_neosho_little_nh.txt_PofZ.txt")

# Read in NewHybrids "PofZ" output txt file for Neosho and Ouachita Bass
ih_neosho_ouachita_pofz <- read_tsv("data/newhybrids_data/output_data/ih_neosho_ouachita_output/ih_neosho_ouachita_nh.txt_PofZ.txt")

# Read in NewHybrids "PofZ" output txt file for Little River and Ouachita Bass
ih_little_ouachita_pofz <- read_tsv("data/newhybrids_data/output_data/ih_little_ouachita_output/ih_little_ouachita_nh.txt_PofZ.txt")

# Omit first two columns (the first column is individual number and the second is empty)
ih_neosho_little_pofz <- data.frame(ih_neosho_little_pofz[,-c(1:2)])
ih_neosho_ouachita_pofz <- data.frame(ih_neosho_ouachita_pofz[,-c(1:2)])
ih_little_ouachita_pofz <- data.frame(ih_little_ouachita_pofz[,-c(1:2)])

# Change column names to indicate hybrid class clearly for Neosho and Little River Bass
colnames(ih_neosho_little_pofz) <- c("pure_neosho","pure_little","f1","f2","bc_neosho","bc_little")

# Change column names to indicate hybrid class clearly for Neosho and Ouachita River Bass
colnames(ih_neosho_ouachita_pofz) <- c("pure_neosho","pure_ouachita","f1","f2","bc_neosho","bc_ouachita")

# Change column names to indicate hybrid class clearly for Little River and Ouachita River Bass
colnames(ih_little_ouachita_pofz) <- c("pure_little","pure_ouachita","f1","f2","bc_little","bc_ouachita")

# Bind metadata to NewHybrids output to match posterior probabilities with individual samples
ih_neosho_little_pofz <- cbind(ih_neosho_little_metadata, ih_neosho_little_pofz)
ih_neosho_ouachita_pofz <- cbind(ih_neosho_ouachita_metadata, ih_neosho_ouachita_pofz)
ih_little_ouachita_pofz <- cbind(ih_little_ouachita_metadata, ih_little_ouachita_pofz)
...
ih_neosho_little_pofz <- ih_neosho_little_pofz %>%
  gather(c(14:19), key = "hybrid_category", value = "probability") %>%
  mutate(hybrid_category = factor(hybrid_category)) %>%
  mutate(Structure_Num = as.character(Structure_Num))

ih_neosho_ouachita_pofz <- ih_neosho_ouachita_pofz %>%
  gather(c(14:19), key = "hybrid_category", value = "probability") %>%
  mutate(hybrid_category = factor(hybrid_category)) %>%
  mutate(Structure_Num = as.character(Structure_Num))

ih_little_ouachita_pofz <- ih_little_ouachita_pofz %>%
  gather(c(14:19), key = "hybrid_category", value = "probability") %>%
  mutate(hybrid_category = factor(hybrid_category)) %>%
  mutate(Structure_Num = as.character(Structure_Num))

# Change levels to get the correct order (see explanation for step above)
ih_neosho_little_pofz$Structure_Num <- factor(ih_neosho_little_pofz$Structure_Num, 
                                              levels = unique(ih_neosho_little_pofz$Structure_Num))

ih_neosho_ouachita_pofz$Structure_Num <- factor(ih_neosho_ouachita_pofz$Structure_Num, 
                                                levels = unique(ih_neosho_ouachita_pofz$Structure_Num))

ih_little_ouachita_pofz$Structure_Num <- factor(ih_little_ouachita_pofz$Structure_Num, 
                                                levels = unique(ih_little_ouachita_pofz$Structure_Num))
...
pdf("/Users/joegunn/Desktop/Postdoc_Stuff/auxillary_projects/SNOLH_Genomics/structure_analysis/figures/newhybrids_figures/ih_neosho_little.pdf", width=120, height=12)

ggplot(ih_neosho_little_pofz, aes(x = Structure_Num, y = probability, fill = hybrid_category)) + 
...
pdf("/Users/joegunn/Desktop/Postdoc_Stuff/auxillary_projects/SNOLH_Genomics/structure_analysis/figures/newhybrids_figures/ih_neosho_ouachita.pdf", width=120, height=12)

ggplot(ih_neosho_ouachita_pofz, aes(x = Structure_Num, y = probability, fill = hybrid_category)) + 
   geom_bar(stat = "identity", color = "black", show.legend = T) +
   theme_set(theme_cowplot(12)) +
   labs(x = "Individual", y = "Cumulative probability of hybrid ID") +
   scale_fill_manual(values = c("violet","springgreen","mediumpurple","lightyellow","deepskyblue","firebrick3")) +
...
pdf("/Users/joegunn/Desktop/Postdoc_Stuff/auxillary_projects/SNOLH_Genomics/structure_analysis/figures/newhybrids_figures/ih_little_ouachita.pdf", width=120, height=12)

ggplot(ih_little_ouachita_pofz, aes(x = Structure_Num, y = probability, fill = hybrid_category)) + 
   geom_bar(stat = "identity", color = "black", show.legend = T) +
   theme_set(theme_cowplot(12)) +
   labs(x = "Individual", y = "Cumulative probability of hybrid ID") +
   scale_fill_manual(values = c("violet","springgreen","mediumpurple","lightyellow","deepskyblue","firebrick3")) +
...
<b>Hybrid individuals between Neosho Bass and Little River Bass: </b> <br>

LEE_010 <br>

<b>Hybrid individuals between Neosho Bass and Ouachita Bass: </b> <br>

No hybrids <br>

<b>Hybrid individuals between Little River Bass and Ouachita Bass: </b> <br>
