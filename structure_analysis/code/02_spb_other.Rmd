---
title: "Analysis 2 - Step 2: Hybridization between Spotted Bass and Cenetral Interior Highlands Species"
author: "Joe Gunn"
date: "2022-08-13"
output: html_document
---

# Project: Cenetral Interior Highlands Hybridization: Spotted, Smallmouth, Neosho, Ouachita, and Little River Bass (SNOLH)
...
## Specific Aim: Hierarchical structure and hybrid detection in Spotted Bass and Cenetral Interior Highlands Species
For this aim, we are using Bayesian inference with STRUCTURE and NEWHYBRIDS to assess patterns and levels of hybridization between Spotted Bass (<i>M. punctulatus</i>) and all other species of interest in the Cenetral Interior Highlands, including Smallmouth Bass (<i>M. dolomieu</i>), Neosho Bass (<i>M. velox</i>), and two potentially distinct species in the Ouachita River Basin (Ouachita Bass and Little River Bass).
...
##### Filter out genotype data into diagnostic SNP groups for Spotted Bass and all other Cenetral Interior Highlands species
```{r}
...
<b>Details on SPB and Cenetral Interior Highlands dataset:</b> <br>
...
```{r}
# Generate structure file for Spotted Bass and all other Cenetral Interior Highlands Bass
...
<b>Details on SMB and Cenetral Interior Highlands populations:</b> <br>
...
<b>SPB and all other Cenetral Interior Highlands populations:</b> 25 <br>
...
We found that Puechmaille metrics universally support K=2, and we graph the results for deltak and Puechmaille in Step 2g.5.
...
spb_other_evanno <- as.data.frame(spb_other_evanno)

## Write Excel file
write_xlsx(spb_other_evanno, "../data/structure_data/deltak_data/spb_other_deltak.xlsx")
...
##### deltak results for SPB and other Central Interior Highlands populations: `02_spb_other_deltaK.pdf` 
```{r}
# DeltaK results
pdf("../figures/structure_figures/deltak_plots/02_spb_other_deltaK.pdf", width=8, height=5)
...
We found strongest support for K=2 using the deltaK metric. We also found strongest support for K=2 using the Puechmaille metric. We therefore present result for K=2 in Step 2g.5 below.

##### 2g.5. Plot STRUCTURE runs at best K value (K=2); run the rmd chunk below.

##### STRUCTURE results of SPB and other Cenetral Interior Highlands populations: `02_spb_other_k2.pdf`
```{r}
...
plotQ(spb_other_k2,
      ...
      height = 10, 
      width = 100,
      outputfilename = "02_spb_other_k2",
      exportpath = "../figures/structure_figures/q_plots")
```

#### 2h: Analyze ancestry proportions from Spotted Bass for all river populations in the Cenetral Interior Highlands; run the Rmd cbhunk below:
In this step, we summarize ancestry proportions for each river calculated in STRUCTURE. Specifically, we calculate the average Spotted Bass ancestry across rivers at K=2.

##### Summarize ancestry proportions for rivers:
```{r}
# column bind metadata with q values determined in STRUCTURE
spb_other_k2 <- cbind(spb_other_metadata, spb_other_k2)

# select only the river column, along with the columns containing q values from structure
spb_other_k2 <- spb_other_k2[,c(7,14:15)]

# rename columns to be more descriptive ("SMBC ancestry" indicates smallmouth bass species complex (Cenetral Interior Highlands) ancestry)
colnames(spb_other_k2) <- c("River", "SMBC_ancestry", "Spotted_Bass_ancestry")

# Omit Spotted Bass individuals (first five rows of dataset)
spb_other_k2 <- spb_other_k2[-c(1:5),]

# Summarize average ancestry of Spotted Bass per river
spb_ancestry_summary <- spb_other_k2 %>%
  group_by(River) %>%
  summarize(mean_spb = mean(Spotted_Bass_ancestry))
...
```{r}
# Generate newhybrids file for Spotted Bass and all other Cenetral Interior Highlands Bass
...
##### Visualize NEWHYBRIDS output and generate plot for SPB and all other Cenetral Interior Highlands species: `spb_other.pdf`
```{r}
...
save(spb_other_pofz, file = "../data/newhybrids_data/output_data/spb_other_output/spb_other_pofz.rda")
...
pdf("../figures/newhybrids_figures/02_spb_other.pdf", width=120, height=12)
...
Before moving on to assess population structure and hybridization in the Smallmouth Bass and the rest of the Cenetral Interior Highlands species (Neosho Bass, Ouachita Bass, and Little River Bass), we are removing any hybrids resulting from crossing with Spotted Bass.
...
spb_other_pure_spb <- spb_other_pofz %>%
...
spb_other_bc_ih <- spb_other_pofz %>%
...
spb_other_bc_spb <- spb_other_pofz %>%
...
spb_other_pure <- rbind(spb_other_pure_ih, spb_other_pure_spb)

# Generate list of "hybrid" samples (f1, f2, and backcrosses)
spb_other_hybrids <- rbind(spb_other_f1, spb_other_f2, spb_other_bc_ih, spb_other_bc_spb)

# Select the first column and convert to data frame
spb_other_hybrids <- data.frame(spb_other_hybrids)

# Save the list of bad samples as a .txt file without column names
write_tsv(spb_other_hybrids, 
          file = "../data/filtering_data/spb_other_hybrids.txt", 
          col_names = FALSE)

# Convert hybrids dataframe to vector for saving and loading later
spb_other_hybrids <- as.vector(spb_other_hybrids[,1])

# Save list to be loaded in 1d_ih.Rmd
save(spb_other_hybrids, file = "../data/filtering_data/spb_other_hybrids.rda")
```
