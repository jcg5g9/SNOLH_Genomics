...
##### Filter metadata to only include Smallmouth Bass and all other Central Interior Highlands species, excluding hybrids with Spotted Bass
```{r}
## get metadata for c-binding later
smb_ih_metadata <- full_data[1:13]

## Filter out Spotted Bass from the metadata
smb_ih_metadata <- smb_ih_metadata %>%
  filter(Putative_Taxon != "Spotted") 

# Remove hybrids detected with Spotted Bass (Analysis 2, Step 1b (1b_spb_other.Rmd))
smb_ih_metadata <- smb_ih_metadata[ ! smb_ih_metadata$Structure_Num %in% spb_other_hybrids, ] 
...
##### Filter out genotype data into diagnostic SNP groups for Smallmouth Bass and all other Central Interior Highlands species
```{r}
...
<b>Details on SMB and Central Interior Highlands dataset:</b> <br>
...
```{r}
# Generate structure file for Smallmouth Bass and all other Central Interior Highlands Bass
...
<b>Details on SMB and Central Interior Highlands populations:</b> <br>
...
##### Generate batch commands: `batch_cmd_lists/smb_ih_structure_batch_cmd_list.txt`
```{r}
# N(pops) = 23
...
cat("", file="batch_cmd_lists/smb_ih_structure_batch_cmd_list.txt")
...
for(ii in 1:nrow(nk)) {
  ...
  for(aa in 1:nrow(nreps)) {
    ...
    cat(paste(structure_call, "-K ", nk[ii,], param_files, input, output,
      ...
      "\n", 
      file=paste("batch_cmd_lists/smb_ih_structure_batch_cmd_list.txt"), # name file to store list
      append=TRUE)
  }
}
```
...
##### Generate batch commands: `shell_scripts/smb_ih_structure.sh`
```{r}
#!/bin/bash
#-------------------------------------------------------------------------------
#  SBATCH CONFIG
...
Run `sbatch smb_ih_structure.sh`
...
smb_ih_evanno <- as.data.frame(smb_ih_evanno)

## Write Excel file
write_xlsx(smb_ih_evanno, "../data/structure_data/deltak_data/smb_ih_deltak.xlsx")
...
##### deltak results for SMB and other Central Interior Highlands populations: `03_smb_ih_deltaK.pdf` 
```{r}
# DeltaK results
pdf("../figures/structure_figures/deltak_plots/03_smb_ih_deltaK.pdf", width=8, height=5)
...
##### STRUCTURE results of SMB and all other Central Interior Highlands populations; 1) `03_smb_ih_k3.pdf` and 2) `03_smb_ih_k4.pdf`
```{r}
# Read in combined-merged Q table from CLUMPP
smb_ih_k3 <- readQ("../data/structure_data/clumpp_data/smb_ih_clumpp/pop_K3/pop_K3-combined-merged.txt")

# Visualize Structure
plotQ(smb_ih_k3,
      grplab = smb_ih[,c(13,7)],
      showindlab = FALSE, 
      ordergrp = F,
      selgrp = "River",
      showgrplab = T,
      ...
      height = 10, 
      width = 100,
      outputfilename = "03_smb_ih_k3",
      exportpath = "../figures/structure_figures/q_plots")

# Read in combined-merged Q table from CLUMPP
smb_ih_k4 <- readQ("../data/structure_data/clumpp_data/smb_ih_clumpp/pop_K4/pop_K4-combined-merged.txt")
...
plotQ(smb_ih_k2,
      ...
      height = 10, 
      width = 100,
      outputfilename = "03_smb_ih_k2",
      exportpath = "../figures/structure_figures/q_plots")

```

#### 2h: Analyze ancestry proportions from Smallmouth Bass for all river populations in the Central Interior Highlands; run the Rmd cbhunk below:
In this step, we summarize ancestry proportions for each river calculated in STRUCTURE. Specifically, we calculate the average Smallmouth Bass ancestry across rivers.

##### Summarize ancestry proportions for rivers:
```{r}
# column bind metadata with q values determined in STRUCTURE
smb_ih_k3 <- cbind(smb_ih_metadata, smb_ih_k3)

# select only the river column, along with the columns containing q values from structure
smb_ih_k3 <- smb_ih_k3[,c(7,14:16)]
...
colnames(smb_ih_k3) <- c("River", "SMB_ancestry", "CIH_ancestry_1", "CIH_ancestry_2")

# Omit Spotted Bass individuals (first five rows of dataset)
smb_ih_k3 <- smb_ih_k3[-c(419:441),]

# Summarize average ancestry of Spotted Bass per river
smb_ancestry_summary_k3 <- smb_ih_k3 %>%
  group_by(River) %>%
...
# column bind metadata with q values determined in STRUCTURE
smb_ih_k2 <- cbind(smb_ih_metadata, smb_ih_k2)

# select only the river column, along with the columns containing q values from structure
smb_ih_k2 <- smb_ih_k2[,c(7,14:15)]

# rename columns to be more descriptive ("SMBC ancestry" indicates smallmouth bass species complex (Central Interior Highlands) ancestry)
colnames(smb_ih_k2) <- c("River", "SMB_ancestry", "CIH_ancestry")

# Omit Spotted Bass individuals (first five rows of dataset)
smb_ih_k2 <- smb_ih_k2[-c(419:441),]

# Summarize average ancestry of Spotted Bass per river
smb_ancestry_summary_k2 <- smb_ih_k2 %>%
  group_by(River) %>%
...
In this step, we are running complementary analyses on the dataset with Smallmouth Bass and all other samples in the software program NewHybrids, which uses Bayesian inference of genotype data to assign individuals to one of six <i>a priori</i> modeled hybrid classes (parent 1, parent 2, F1 hybrid, F2 hybrid, backcross to parent 1, or backcross to parent 2).
...
```{r}
# Generate newhybrids file for Smallmouth Bass and  other Central Interior Highlands Bass
...
##### Visualize NEWHYBRIDS output and generate plot for SMB and all other Central Interior Highlands species: `smb_ih.pdf`
```{r}
...
save(smb_ih_pofz, file = "../data/newhybrids_data/output_data/smb_ih_output/smb_ih_pofz.rda")
...
pdf("../figures/newhybrids_figures/03_smb_ih.pdf", width=120, height=12)
...
Before moving on to assess population structure and hybridization among Central Interior Highlands species (Neosho Bass, Ouachita Bass, and Little River Bass), we are removing any hybrids resulting from crossing with Smallmouth Bass.
...
smb_ih_hybrid_summary <- smb_ih_pofz %>%
  filter(probability > 0.5) %>%
  group_by(River, hybrid_category) %>%
  count()

# Get pure IH individuals 
smb_ih_pure_ih <- smb_ih_pofz %>%
  filter(hybrid_category == "pure_ih") %>%
  filter(probability > 0.5) %>%
  select(Structure_Num)

### 396 pure IH individuals

# Get pure SMB individuals
smb_ih_pure_smb <- smb_ih_pofz %>%
...
# Gets IH backcross individuals
smb_ih_bc_ih <- smb_ih_pofz %>%
  filter(hybrid_category == "bc_ih") %>%
  filter(probability > 0.5) %>%
  select(Structure_Num)
...
smb_ih_pure <- rbind(smb_ih_pure_ih, smb_ih_pure_smb)

# Generate list of "hybrid" samples (f1, f2, and backcrosses)
smb_ih_hybrids <- rbind(smb_ih_f1, smb_ih_f2, smb_ih_bc_ih, smb_ih_bc_smb)

# Select the first column and convert to data frame
smb_ih_hybrids <- data.frame(smb_ih_hybrids)

# Save the list of bad samples as a .txt file without column names
write_tsv(smb_ih_hybrids, 
          file = "../data/filtering_data/smb_ih_hybrids.txt", 
          col_names = FALSE)

# Convert hybrids dataframe to vector for saving and loading later
smb_ih_hybrids <- as.vector(smb_ih_hybrids[,1])

# Save list to be loaded in 03_snolh_structure_ih.Rmd
save(smb_ih_hybrids, file = "../data/filtering_data/smb_ih_hybrids.rda")
```
