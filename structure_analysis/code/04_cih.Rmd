...
cih_metadata <- cih_metadata[ ! cih_metadata$Structure_Num %in% spb_other_hybrids, ] 
...
# Remove hybrids detected with SPB (Analysis 2, Step 1b (1b_spb_other.Rmd))
cih_snps <- cih_snps[ ! cih_snps$Structure_Num %in% spb_other_hybrids, ]

# Remove hybrids detected with SMB (Analysis 2, Step 1c (1b_smb_ih.Rmd))
...
ih <- cbind(cih_metadata, cih_snps)
...
nb_lrb_pofz$Structure_Num <- factor(nb_lrb_pofz$Structure_Num, 
                                              levels = unique(nb_lrb_pofz$Structure_Num))

nb_ob_pofz$Structure_Num <- factor(nb_ob_pofz$Structure_Num, 
                                                levels = unique(nb_ob_pofz$Structure_Num))

lrb_ob_pofz$Structure_Num <- factor(lrb_ob_pofz$Structure_Num, 
                                                levels = unique(lrb_ob_pofz$Structure_Num))
...
ggplot(nb_lrb_pofz, aes(x = Structure_Num, y = probability, fill = hybrid_category)) + 
...
ggplot(nb_ob_pofz, aes(x = Structure_Num, y = probability, fill = hybrid_category)) + 
   geom_bar(stat = "identity", color = "black", show.legend = T) +
   theme_set(theme_cowplot(12)) +
   labs(x = "Individual", y = "Cumulative probability of hybrid ID") +
   scale_fill_manual(values = c("blue","red","green","yellow","white","white")) +
...
ggplot(lrb_ob_pofz, aes(x = Structure_Num, y = probability, fill = hybrid_category)) + 
   geom_bar(stat = "identity", color = "black", show.legend = T) +
   theme_set(theme_cowplot(12)) +
   labs(x = "Individual", y = "Cumulative probability of hybrid ID") +
   scale_fill_manual(values = c("blue","red","green","yellow","white","white")) +
