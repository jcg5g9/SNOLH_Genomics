...
##### 2a.1. Format data, calculate genotype missigness and plot raw results; run the Rmd chunk below.
...
##### 2b.1. Format data, calculate genotype missigness and plot raw results; run the Rmd chunk below.
...
### STEP 3. Filter loci and samples with poor genotype quality (high missigness rates across samples and loci).
...
In this step, we summarize genotype missigness for loci across samples and for samples across loci to determine the effectiveness of the 20% missing call rate threshold. We also assess whether genotype missigness is clustered by important groups in the metadata, e.g., by species or genetic clusters inferred in downstream analyses.

#### 5a. Calculate genotype missigness for loci across samples in the fully filtered dataset; run the Rmd chunk below. 

##### Calculate genotype missingness for loci across samples in the filtered dataset:
```{r}
# Load genotype data with Shoal Bass filter (gdata_01_filter_shoal.Rda)
load("data/processed_raw_data/gdata_04_filter_duplicates.Rda")

# Get counts of "no-genotype" calls per site across samples
snp_missing <- apply(gdata_04_filter_duplicates[2:187], 2, function(x) {length(which(x=="unknown"))})
...
final_snp_missing <- snp_missing %>%
  mutate(missing_rate = snp_missing/472)

# Save final SNP missingness data 
save(final_snp_missing, file = "data/filtering_data/final_snp_missing.Rda")
```

#### 5b. Calculate genotype missigness for samples across loci in the fully filtered dataset; run the Rmd chunk below. 
