...
For this aim, we clean and filter the full genotype data for 487 black bass individuals, which was derived from the diagnostic SNP panel developed by Long et al. (2021) and prepare the data for analysis in Structure and NewHybrids (See Analysis 2).

Specifically, we filter the dataset based on three criteria:

  1) out poor quality SNP loci (loci that failed to genotype in over 20% of samples);
...
pdf("figures/snp_missing.pdf", width=7, height=5)

ggplot(snp_missing, aes(missing_rate)) + 
  geom_histogram(binwidth = 0.01) + 
  geom_vline(xintercept = 0.20, color = "red", linetype = "longdash", size = 2) +
  theme_set(theme_cowplot(12)) +
  labs(x = "Missing genotype call rate across samples (%)", y = "NSNP") +
  theme(axis.title = element_text(size = 15)) +
  theme(axis.text = element_text(size = 15)) +
  ylab(expression(N[SNP]))
...
# Plot histogram of missing genotype call rates across samples for the dataset
pdf("figures/sample_missing.pdf", width=7, height=5)

ggplot(sample_missing, aes(missing_rate)) + 
  geom_histogram(binwidth = 0.01) + 
  geom_vline(xintercept = 0.20, color = "red", linetype = "longdash", size = 2) +
  theme_set(theme_cowplot(12)) +
  labs(x = "Missing genotype call rate across samples (%)", y = "Nsample") +
  theme(axis.title = element_text(size = 15)) +
  theme(axis.text = element_text(size = 15)) +
  ylab(expression(N[sample]))
