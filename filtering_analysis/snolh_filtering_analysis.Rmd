...
save(snp_missing, file = "data/filtering_data/snp_missing.Rda")
...
load("data/filtering_data/snp_missing.Rda")

# Calculate average number of SNPs missing per locus across samples
snp_missing %>%
  summarize(mean_snp_missing = mean(snp_missing))
...
sample_missing <- apply(gdata_01_filter_shoal, 1, function(x) {length(which(x=="unknown"))})

# Convert to dataframe 
sample_missing <- as.data.frame(sample_missing)

# Bind with sample ID
sample_missing <- cbind(gdata_01_filter_shoal$Sample_ID, sample_missing)
...
sample_missing <- sample_missing %>%
  mutate(missing_rate = sample_missing/192)

# Save sample missingness data for summary analysis (see Step 2b.2 below)
save(sample_missing, file = "data/filtering_data/sample_missing.Rda")
...
```{r}
# Load SNP missingness data 
load("data/filtering_data/sample_missing.Rda")
...
#### 5a. Calculate genotype missingness for SPB and SMBC. 
In this step, we calculate missingness across loci and individuals for only the SPB and SMBC dataset (all individuals but only SPB loci).

##### 5a.1. Calculate missingness for SPB loci across all samples.
...
# Get counts of "no-genotype" calls per site across samples
snp_missing <- apply(spb_data, 2, function(x) {length(which(x=="unknown"))})
...
save(snp_missing, file = "data/filtering_data/spb_smbc_snp_missing.Rda")
```

<b>Post-filtering missingness for SPB and SMBC loci:</b> <br>
<i>N</i>Mean number of missing genotypes = 2.666667 <br>
<i>N</i>Mean missingness rate = 0.005649718 <br>

##### 5a.2. Calculate missingness for all samples across only SPB loci.
...
# Get counts of "no-genotype" calls per site across samples
sample_missing <- apply(spb_data, 1, function(x) {length(which(x=="unknown"))})

# Convert to dataframe
sample_missing <- as.data.frame(sample_missing)

# Bind with sample ID
sample_missing <- cbind(filtered_data[,c(1,5)], sample_missing)
...
save(sample_missing, file = "data/filtering_data/spb_smbc_sample_missing.Rda")
```

<b>Post-filtering missingness for SPB and SMBC sample:</b> <br>
<i>N</i>Mean number of missing genotypes = 0.08474576 <br>
<i>N</i>Mean missingness rate = 0.005649718 <br>

##### 5a.3. Compare genotype missingness across loci between SPB and SMBC groups to assess potential bias.
...
mod <- lm(missing_rate ~ group, data = sample_missing)
