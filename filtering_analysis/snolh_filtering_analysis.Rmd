...
For this aim, we clean and filter the full genotype data for 487 black bass individuals, which was derived from the diagnostic SNP panel developed by Long et al. (2021). Specifically, we filter out poor quality SNP loci (loci that failed to genotype in over 20% of samples) and poor quality samples (samples that failed to genotype in over 20% of loci). We addtionally filter for potential duplicate samples and prepare the data for analysis in Structure and NewHybrids (See Analysis 2).

## Phases of Analysis
### Phase 1: Data cleaning and filtering
...
In this step, we are reading in the raw data file (which includes sample metadata and SNP genotype data; 'snolh_all_data.xlsx'), manipulating and cleaning the data, and generating separate data frames for the metadata (without SNP genotype data) and for the SNP genotype data (without metadata).

#### 1a: Read in, clean, and save full dataset. 
As part of data cleaning, we are omitting three samples representing Shoal Bass (<i>M. cataractae</i>) due to low sample size and because we are not explicitly interested in Shoal Bass hybridization or population structure. Additionally, we remove SNP markers with low call rates across samples and samples with low genotyping rates across SNPs.
...
#### Filter SNPs with low genotype call rates across samples and viusualize missingness results: `figures/filtering_figures/
```{r}
# Get counts of "no-genotype" calls per site across samples
snp_missing <- apply(clean_data[2:193], 2, function(x) {length(which(x=="unknown"))})
...
##### Save fully filtered, clean dataset: `../processed_raw_data/snolh_clean_data.Rda` 
```{r}
# Save full, clean dataset as .Rda file to be loaded in downstream analysis
save(clean_data, file = "../processed_raw_data/snolh_clean_data.rda")
```
...
# Convert characters, etc. to factors
metadata <- metadata %>%
  mutate(Sample_ID = factor(Sample_ID),
         Ref_samps = factor(Ref_samps),
...
save(metadata, file = "../processed_raw_data/snolh_metadata.rda")
```

##### 1b.1. The metadata is saved as the .Rda file `../processed_raw_data/snolh_metadata.rda`, which can be loaded independently for downstream analyses. The associated R object is called "metadata"
...
load("../processed_raw_data/snolh_clean_data.rda")

# Get genotypes only from all data
genotype_data <- clean_data[,2:187]
...
genotype_data <- cbind(clean_data[,1], genotype_data)

# Save SNP genotype data as .Rda file to be loaded in downstream analysis
save(genotype_data, file = "../processed_raw_data/snolh_genotype_data.rda")
```

##### 1c.1. The SNP genotype data is saved as the .Rda file `../processed_raw_data/snolh_genotype_data.rda`, which can be loaded independently for downstream analyses. The associated R object is called "genotype_data".
...
```{r}
# Load "genotype_data", and "metadata" from .Rda files
load("../processed_raw_data/snolh_metadata.rda")
load("../processed_raw_data/snolh_genotype_data.rda")

# cbind metadata and genotype data
full_data <- merge(metadata, genotype_data, by = "Sample_ID")

# Arrange data by the second column (geographic order)
full_data <- full_data %>%
  arrange(Analysis_order_W2E_U2D_corrected)

# save the full dataset for future analyses
save(full_data, file = "../processed_raw_data/snolh_full_data.rda")
```

##### 1d.1. The cleaned full dataset is saved as the .Rda file `../raw_data/snolh_full_data.rda`, which can be loaded independently for downstream analyses. The associated R object is called "full_data".
