...
In this step, we use DAPC to cluster samples by hierarhcical group (SPB vs. SMB-C; SMB vs. CIH; and CIH only) both including and excluding missing data to see if there is a substantial difference in clustering due to differences in missingness.

#### 2a. Generate data frames of genotypes and group names for all data (including missing genotypes) and for all data excluding missing genotypes; run the Rmd chunk below.
...
#### 2b. Convert to genind objects and scale matrix values for each dataset generated in Step 2a above; run the Rmd chunk below.
...
#### 2c. Run DAPC for both datasets (with and without missing data); run the Rmd chunk below.
...
missing_dapc <- dapc(missing_genind, spb_smbc_genotypes_missing_spp)

# Run DAPC for SPB vs. SMBC (without missing daa) at inferred optimal number of PCs
no_missing_dapc <- dapc(no_missing_genind, spb_smbc_genotypes_no_missing_spp)
...
#### 2d. Extract linear discriminant (LD) results from DAPC for each dataset run in Step 2c above; run the Rmd chunk below.

##### Extract LD results from DAPC:
```{r}
# Load DAPC for SPB data (with missing genotypes)
load("data/validation_data/missing_dapc.Rda")

# Load DAPC for SPB data (without missing genotypes)
load("data/validation_data/no_missing_dapc.Rda")
...
#### 2d. Plot LD data from DAPC for both datasets; run the Rmd chunk below.
