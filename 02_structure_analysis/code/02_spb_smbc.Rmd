...
## PHASE 2: STRUCTURE AND HYBRID ASSIGNMENT ANALYSIS OF SPB AND SMBC
...
#### 1a: Subset the data to include only individuals for the full population structure analysis and SPB diagnostic SNPs
...
##### 1a.1. Subset full data to include only SPB-SNPS; run the Rmd chunk below:
...
save(spb_smbc_metadata, file = "data/spb_smbc_metadata.Rda")
...
save(spb_smbc_snps, file = "data/spb_smbc_snps.Rda")
```
...
#### 1b: Combine metadata and snp datasets for combination of interest (see above); run the Rmd chunk below to combine datasets.

##### Combine metadata and snp datasets and generate Excel file, which is stored in the directory `data/processed_genotype_data/`
```{r}
# load metadata
load("data/spb_smbc_metadata.Rda")
...
save(spb_smbc_evanno, file = "../data/structure_data/deltak_data/spb_smbc_evanno.Rda")
...
save(spb_smbc_align, file = "../data/structure_data/aligned_data/spb_smbc_align.Rda")
```
...
load("../data/structure_data/aligned_data/spb_smbc_align.Rda")
...
```{r}
# Load deltaK data
load("../data/structure_data/deltak_data/spb_smbc_evanno.Rda")
...
```{r}
# define color palette for structure diagram
...
## Generate excel files to hold genotype data
write_xlsx(spb_smbc_pofz, path = "../../../../../spb_smbc.xlsx")
```
