...
## PHASE 4: STRUCTURE AND HYBRID ASSIGNMENT ANALYSIS OF SPECIES IN THE CIH
...
save(cih_metadata, file = "data/cih_metadata.Rda")
```

##### 1a.2. Filter SNP genotype data; run the Rmd chunk below:
...
save(cih_snps, file = "data/cih_snps.Rda")
```
...
```{r}
# load metadata
load("data/cih_metadata.Rda")

# load snp data
load("data/cih_snps.Rda")
...
##### 1c.1. Follow the guide for the library 'genepopedit' (Stanley et al. 2017) to prepare genepop files before converting them to STRUCTURE format.
...
###### 1.e.1.2. Set the number of burn-in and MCMC iterations to run; these are the same for each analysis:
...
save(cih_evanno, file = "../data/structure_data/deltak_data/cih_evanno.Rda")

# Align replicate runs for each K to correct label switching
cih_align <- alignK(cih_Q)

# Save aligned data for clumpp analysis
save(cih_align, file = "../data/structure_data/aligned_data/cih_align.Rda")
```
...
load("../data/structure_data/aligned_data/cih_align.Rda")
...
```{r}
# Load deltaK data
load("../data/structure_data/deltak_data/cih_evanno.Rda")
...
nb_lrb_metadata <- cih_metadata %>%
  filter(putative_taxon != "OB")

# metadata for NB and OB
nb_ob_metadata <- cih_metadata %>%
  filter(putative_taxon != "LRB")

# metadata for LRB and OB
lrb_ob_metadata <- cih_metadata %>%
  filter(putative_taxon != "NB") 

# save nb_lrb metadata for downstream analysis
save(nb_lrb_metadata, file = "data/nb_lrb_metadata.Rda")

# save nb_ob metadata for downstream analysis
save(nb_ob_metadata, file = "data/nb_ob_metadata.Rda")

# save lrb_ob metadata for downstream analysis
save(lrb_ob_metadata, file = "data/lrb_ob_metadata.Rda")
```
...
```{r}
# load nb_lrb metadata 
load("data/nb_lrb_metadata.Rda")

# save nb_ob metadata 
load("data/nb_ob_metadata.Rda")

# save lrb_ob metadata
load("data/lrb_ob_metadata.Rda")
...
# Bind metadata to NewHybrids output to match posterior probabilities with nb_lrb samples
nb_lrb_pofz <- cbind(nb_lrb_metadata, nb_lrb_pofz)

# Bind metadata to NewHybrids output to match posterior probabilities with nb_ob samples
nb_ob_pofz <- cbind(nb_ob_metadata, nb_ob_pofz)

# Bind metadata to NewHybrids output to match posterior probabilities with lrb_ob samples
lrb_ob_pofz <- cbind(lrb_ob_metadata, lrb_ob_pofz)

# Gather nb_lrb probability dataset for plotting and convert variables to factors/characters. 'structure_number' was converted to a character in this step, and then factor levels were set in the next step to ensure that individuals were graphed in the same order as in STRUCTURE (for later direct one-to-one comparison)
nb_lrb_pofz <- nb_lrb_pofz %>%
  gather(c(14:19), key = "hybrid_category", value = "probability") %>%
  mutate(hybrid_category = factor(hybrid_category)) %>%
  mutate(structure_number = as.character(structure_number))

# Gather nb_ob probability dataset for plotting and convert variables to factors/characters. 'structure_number' was converted to a character in this step, and then factor levels were set in the next step to ensure that individuals were graphed in the same order as in STRUCTURE (for later direct one-to-one comparison)
nb_ob_pofz <- nb_ob_pofz %>%
  gather(c(14:19), key = "hybrid_category", value = "probability") %>%
  mutate(hybrid_category = factor(hybrid_category)) %>%
  mutate(structure_number = as.character(structure_number))

# Gather lrb_ob probability dataset for plotting and convert variables to factors/characters. 'structure_number' was converted to a character in this step, and then factor levels were set in the next step to ensure that individuals were graphed in the same order as in STRUCTURE (for later direct one-to-one comparison)
lrb_ob_pofz <- lrb_ob_pofz %>%
  gather(c(14:19), key = "hybrid_category", value = "probability") %>%
  mutate(hybrid_category = factor(hybrid_category)) %>%
  mutate(structure_number = as.character(structure_number))

# Change levels to get the correct order (see explanation for step above) for nb_lrb data
...
# Change levels to get the correct order (see explanation for step above) for nb_ob data
...
# Change levels to get the correct order (see explanation for step above) for lrb_ob data
...
# Convert analysis order column to factor for nb_lrb data
nb_lrb_pofz <- nb_lrb_pofz %>%
  mutate(analysis_order = factor(analysis_order))

# Convert analysis order column to factor for nb_ob data
nb_ob_pofz <- nb_ob_pofz %>%
  mutate(analysis_order = factor(analysis_order))

# Convert analysis order column to factor for lrb_ob data
lrb_ob_pofz <- lrb_ob_pofz %>%
  mutate(analysis_order = factor(analysis_order))

# Reorder analysis order to be in the same order as rivers (same as presented in Structure q plot) for nb_lrb data
nb_lrb_pofz <- nb_lrb_pofz %>% 
  mutate(analysis_order = fct_reorder(analysis_order, as.integer(river)))

# Reorder analysis order to be in the same order as rivers (same as presented in Structure q plot) for nb_ob data
nb_ob_pofz <- nb_ob_pofz %>% 
  mutate(analysis_order = fct_reorder(analysis_order, as.integer(river)))

# Reorder analysis order to be in the same order as rivers (same as presented in Structure q plot) for lrb_ob data
lrb_ob_pofz <- lrb_ob_pofz %>% 
  mutate(analysis_order = fct_reorder(analysis_order, as.integer(river)))

# Save nb_lrb dataframe for future use in introgress analysis (Analysis 3)
save(nb_lrb_pofz, file = "../data/newhybrids_data/output_data/nb_lrb_output/nb_lrb_pofz.rda")

# Save nb_ob dataframe for future use in introgress analysis (Analysis 3)
save(nb_ob_pofz, file = "../data/newhybrids_data/output_data/nb_ob_output/nb_ob_pofz.rda")

# Save lrb_ob dataframe for future use in introgress analysis (Analysis 3)
save(lrb_ob_pofz, file = "../data/newhybrids_data/output_data/lrb_ob_output/lrb_ob_pofz.rda")

# Plot posterior probability of assignment to each hybrid category as a cumulative bar plot for nb_lrb individuals (colors represent assignment probabilities for each of six possible hybrid categories)
...
# Plot posterior probability of assignment to each hybrid category as a cumulative bar plot for nb_ob individuals (colors represent assignment probabilities for each of six possible hybrid categories)
...
# Plot posterior probability of assignment to each hybrid category as a cumulative bar plot for lrb_ob individuals (colors represent assignment probabilities for each of six possible hybrid categories)
...
```{r}
# load nb_lrb data 
load("../data/newhybrids_data/output_data/nb_lrb_output/nb_lrb_pofz.rda")

# load nb_lrb data 
load("../data/newhybrids_data/output_data/nb_ob_output/nb_ob_pofz.rda")

# load nb_lrb data 
load("../data/newhybrids_data/output_data/lrb_ob_output/lrb_ob_pofz.rda")
...
Note: there is a single Backcross to Little River in the Ouachita range (Western Saline River)
