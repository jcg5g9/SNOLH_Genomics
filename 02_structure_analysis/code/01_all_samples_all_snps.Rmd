...
In this phase of analysis, we are screening population structure and hybridization among SPB, SMB, NB, OB, and LRB using all SNPs in the Long et al. (2020) SNP panel. We use the program STRUCTURE to estimate ancestry coefficients and assess overall population structure in SPB and SMBC.

### STEP 1: Data preparation

#### 1a: Format and convert full data to Excel; run the Rmd chunk below:
...
##### 1b.1. Remove all metadata columns from the dataset except for "river", which designates the a priori stream population for a given sample, and "structure_number", which gives a structure-formatted individual label for each sample

##### 1b.2. Add a "," after each structure_number

##### 1b.3. Insert a row above each new population in the data, and insert "pop" in the structure_number column in each empty row (the word "pop" should appear for each population)

##### 1b.4. Delete the "river" column
...
save(all_evanno, file = "../data/structure_data/deltak_data/all_evanno.Rda")

# Align replicate runs for each K to correct label switching
all_align <- alignK(all_Q)

# Save aligned data for clumpp analysis
save(all_align, file = "../data/structure_data/aligned_data/all_align.Rda")
```
...
load("../data/structure_data/aligned_data/all_align.Rda")
...
```{r}
# Load deltaK data
load("../data/structure_data/deltak_data/all_evanno.Rda")
